...
async def lifespan(app: FastAPI):
    setup_logging("testcases", level=LOG_LEVEL, json_output=LOG_FORMAT_JSON)
    validate_settings()
    await ensure_indexes(get_db(), ["testcases"])
    logger.info("Testcases service ready")
    yield
    await close_client()
    logger.info("Testcases service stopped")
...
@app.post("/testcases/{testcase_id}/approve", response_model=TestcaseOut, tags=["testcases"], summary="Approve Test Case")
async def approve_testcase(testcase_id: str, reviewed_by: str = Query(..., description="Username of the reviewer")):
    db = get_db()
...
@app.post("/testcases/{testcase_id}/reject", response_model=TestcaseOut, tags=["testcases"], summary="Reject Test Case")
async def reject_testcase(testcase_id: str, reviewed_by: str = Query(..., description="Username of the reviewer")):
    db = get_db()
    result = await db[COL].find_one_and_update(
        {"_id": oid(testcase_id)},
