"""
Agent Memory — persistent learning store for the orchestrator.

Records what worked / failed across generations and executions so the
system improves over time.  All data lives in the `agent_memory` MongoDB
collection.

Memory types:
  - selector_pattern : tracks which selectors succeed/fail per page
  - generation_outcome : per-generation quality metrics
  - prompt_variant : tracks prompt template performance
  - repair_insight : patterns learned from successful repairs
"""
...
async def record_selector_outcome(
    ...
    *,
    ...
    """Upsert a selector pattern outcome for a specific page."""
...
async def record_generation_outcome(
    ...
    *,
    ...
    """Record the outcome of a full generate‑execute cycle."""
    doc = {
...
async def record_repair_insight(
    ...
    *,
    ...
    """Record a repair strategy that worked (or didn't)."""
...
async def get_selector_insights(
    ...
    """Return top selector patterns (optionally filtered by page), sorted by failure count desc."""
...
async def get_generation_stats(
    ...
    """Aggregate generation success/failure stats for the last N days."""
...
async def get_repair_insights(
    ...
    """Return the most common repair patterns sorted by success count."""
    cursor = (
        db[COLLECTION]
...
async def build_memory_prompt_block(
    ...
    """Build a prompt block summarising past learnings for injection into LLM prompts.

    This is the key integration point: the generator calls this before crafting
    its LLM prompt and appends the result so the model benefits from past experience.
    """
    lines: list[str] = []

    # Selector insights for the target page
